...
**飞轮定位**：这是飞轮的**进化引擎**。它消费 harness-resume 积累的执行日志，产出 CLAUDE.md 改进、review-log.md 更新、以及 Skill 流程优化建议，供 harness-plan（规划时避坑）和 harness-init（重新初始化时纳入历史经验）消费。
...
└── Probe 有效性：Probe 推翻假设的比率、Probe 对方案选择的实际影响
```

**Probe 模式分析**（从 phase-journal.jsonl 的 `probes` 字段提取）：

| 分析项 | 指标 | 改进方向 |
|--------|------|---------|
| Probe 使用率 | 含 Probe 的 Phase 占比 | 如果涉及外部系统/性能的 Phase 缺少 Probe → 建议在 plan 中增加 Probe 步骤 |
| 假设推翻率 | `result=refuted` 的占比 | 高推翻率说明 Probe 价值大，应更积极使用；低推翻率（<10%）说明假设质量高或 Probe 过于保守 |
| Probe 遗漏检测 | 无 Probe 但 retries≥3 的 Phase | 这些 Phase 可能因未验证假设而反复重试，建议回溯补充 Probe 规则 |
...
**Skill 使用模式分析**（从 phase-journal.jsonl 中提取）：

| 分析项 | 指标 | 改进方向 |
|--------|------|---------|
| Phase 粒度 | 平均文件变更数、平均重试次数 | 如果平均变更 >8 或平均重试 >2 → 建议 harness-plan 缩小 Phase 范围 |
| 会话切换频率 | 连续在同一会话完成的 Phase 数 | 如果经常连续完成 2+ Phase → 说明 Phase 粒度偏小，可适当合并 |
| Phase 跳过/重做 | 是否有 Phase 被标记跳过或重新执行 | 如果有 → 说明 harness-plan 的依赖分析需加强 |
| 自动 review 命中率 | Step 5 轻量级 review 发现问题的比率 | 如果 >50% Phase 触发 CLAUDE.md 更新 → 说明 harness-init 初始化不够完善 |

如果发现 Skill 使用模式有优化空间，在诊断报告的"具体改进项"中标注来源为"Skill 流程分析"，并给出具体的 SKILL.md 修改建议（由用户决定是否应用）。

**五条铁律合规检查**：

回顾历史 Phase 执行数据，检查是否存在铁律违规：

| 铁律 | 检查方式 | 违规后果 |
|------|---------|---------|
| Mock 过深 = 未测试 | 检查测试文件中 mock 的目标是否有对应 smoke test | 自动化测试通过但手动执行失败 |
| 编译通过 ≠ 功能正确 | `grep -r 'MOCK_\|mock' frontend/src/app/` | 前端页面显示假数据 |
| 环境假设必须验证 | 检查 plan 中外部依赖假设是否在 Phase 开始时验证 | 容器/CLI/端口不存在导致运行时失败 |
| Schema 改动必须 HTTP 验证 | 检查 schemas 改动后是否有 curl 验证记录 | 请求 body 解析错误、字段冲突 |
| 多模块集成必须 smoke test | 检查多模块 Phase 是否有端到端测试 | 模块间接口不一致 |

如果发现铁律违规，在诊断报告中标注为 **P0 问题**，并建议在 CLAUDE.md 中添加对应禁令。
...
- 是 Skill 流程本身有摩擦？→ 建议修改 SKILL.md
- **是铁律未被遵守？→ 检查 harness-resume Step 5b 是否执行了铁律检查**
...
| 指标 | 数值 | 判断 |
|------|------|------|
| 平均 Phase 文件变更数 | [N] | 合理(3-7) / 偏大(8+，建议缩小Phase) / 偏小(<3，可合并Phase) |
| 同会话连续完成 Phase 数 | [N] | 正常(1) / 频繁连续(2+，Phase 粒度可能偏小) |
| CLAUDE.md 自动更新率 | [N%] | 正常(<30%) / 偏高(>50%，init 不够完善) |
